        ...
        for i, j in flatten(extract_search_params(query_dict)).items():
            ...
            if i in searchable_fields:
                if i == "meta_info_language":
                    for field_name, raw_value in flatten(
                        extract_search_params(query_dict)
                    ).items():
                        lang_codes = str(raw_value).split(",")
                        lang_codes = [code.strip() for code in lang_codes]

                        if len(lang_codes) == 1:
                            queryset_dict[f"{search_field_name}__{field_name}"] = (
                                lang_codes[0]
                            )
                        else:
                            queryset_dict[f"{search_field_name}__{field_name}__in"] = (
                                lang_codes
                            )
                        continue
