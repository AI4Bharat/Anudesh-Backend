...
class OrganizationViewSet(viewsets.ModelViewSet):
    ...
    def performance_analytics_data(self, request, pk=None):
        ...
        if lang not in languages:
            return Response(
                {"message": "No data found for selected language"},
                status=status.HTTP_404_NOT_FOUND,
            )
        ...
        for period in range(len(periodical_list) - 1):
            ...
            if metainfo == True:
                ...
                if project_type in get_audio_project_types():
                ...
                elif "InstructionDrivenChat" in project_type or "MultipleLLMInstructionDrivenChat" in project_type:
                    ...
                    # ❗ NO annotation_type filter

                    for anno in annotations:
                        meta = anno.meta_stats or {}
                        total_word_count += meta.get("prompts_word_count", 0)
                        total_turns += meta.get("number_of_turns", 0)

                    result = {
                        "language": lang,
                        "periodical_word_count": total_word_count,
                        "periodical_turns": total_turns,
                        "periodical_tasks_count": annotations.count(),
                        "periodical_aud_duration": "00:00:00",
                    }
            ...
            if metainfo == True and not (
                ...
                or (
                    project_type in get_translation_dataset_project_types()
                    or "ConversationTranslation" in project_type
                    or "InstructionDrivenChat" in project_type
                    or "MultipleLLMInstructionDrivenChat" in project_type
                )
            ):
                period_result = []
