...
from loging.serializers import TransliterationSerializer, TranscriptionSerializer
...
class TranscriptionSelectionViewSet(APIView):
    ...
    def post(self, request):
        ...
        if serializer.is_valid():
            ...
            try:
                if not test_container_connection(
                    TR_AZURE_STORAGE_CONNECTION_STRING, TRANSCRIPTION_CONTAINER_NAME
                ):
                    return Response(
                        {
                            "message": "Failed to establish the connection with the blob container"
                ...
                blob_service_client = BlobServiceClient.from_connection_string(
                    TR_AZURE_STORAGE_CONNECTION_STRING
                )
                container_client = blob_service_client.get_container_client(
                    TRANSCRIPTION_CONTAINER_NAME
                )
                blob_client = container_client.get_blob_client(log_file_name)
    ...
    def get(self, request):
        try:
            ...
            sas_token = generate_blob_sas(
                container_name=TRANSCRIPTION_CONTAINER_NAME,
                blob_name=blob_client.blob_name,
                account_name=account_name,
                account_key=account_key,
                permission=BlobSasPermissions(read=True),
            ...
            blob_url = f"https://{account_name}.blob.{endpoint_suffix}/{TRANSCRIPTION_CONTAINER_NAME}/{blob_client.blob_name}?{sas_token}"
            result = send_email_with_url.delay(user.email, blob_url)
            task_status = result.get()
